...
class UserManager extends Manager
{
	...
	public function getUserByUsernameOrEmail($usernameOrEmail)
	{
		...
		$sql = "SELECT * FROM " . $app->getConfig('security_user_table') .
				" WHERE " . $app->getConfig('security_username_property') . " = :pseudo OR " .
				$app->getConfig('security_email_property') . " = :login LIMIT 1";
		$dbh = ConnectionManager::getDbh();
		$sth = $dbh->prepare($sql);
		$sth->bindValue(":pseudo", $usernameOrEmail);
		$sth->bindValue(":login", $usernameOrEmail);
		if ($sth->execute()){
			$foundUser = $sth->fetch();
			if ($foundUser){
				return $foundUser;
	...
	*/
	public function emailExists($login)
	{

	   $app = getApp();

	   $sql = "SELECT ".$app->getConfig('security_email_property')." FROM " . $app->getConfig('security_user_table') .
	           " WHERE " . $app->getConfig('security_email_property') . " = :login LIMIT 1";
	   $dbh = ConnectionManager::getDbh();
	   $sth = $dbh->prepare($sql);
	 ...
	 * @return boolean true si présent en base de données, false sinon
	 */
	public function usernameExists($pseudo)
	{

	    $app = getApp();

	    $sql = "SELECT ".$app->getConfig('security_username_property')." FROM " . $app->getConfig('security_user_table') .
	            " WHERE " . $app->getConfig('security_username_property') . " = :pseudo LIMIT 1";
	    $dbh = ConnectionManager::getDbh();
	    $sth = $dbh->prepare($sql);
